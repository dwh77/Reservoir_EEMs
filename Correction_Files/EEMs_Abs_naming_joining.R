...
list.files(path = "./Processed_Data/20240725_DH", pattern = "results*", full.names = T)

results <- list.files(path = "./Processed_Data/20240725_DH", pattern = "results*", full.names = T) |> 
...
names <- results |> 
  ...
  mutate(Rep = ifelse(Rep == "r1", 1, 2)) |> #update rep values 
...
#### Bring in Abs values ----
run_date <- "20240725"
abs <- read.csv("./Processed_Data/20240725_DH/CDOM_all_20240725.csv")
...
write.csv(export, "./Processed_Data/20240725_DH/Results_EEMs_Abs_20240725.csv", row.names = F)
