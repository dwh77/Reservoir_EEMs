...
sensor_22may24 <- rbind(ctd_bind, ysi_bind) |> 
  mutate(Distance = ifelse(Site == 101, 0, NA),
         Distance = ifelse(Site == 100, 407, Distance),
         Distance = ifelse(Site == 98, 619, Distance),
         Distance = ifelse(Site == 96, 790, Distance),
         Distance = ifelse(Site == 94, 1015, Distance),
         Distance = ifelse(Site == 92, 1132, Distance),
         Distance = ifelse(Site == 90, 1843, Distance),
         Distance = ifelse(Site == 88, 3326, Distance),
         Distance = ifelse(Site == 50, 5011, Distance)
         ) |> 
  select(Reservoir, Site, Date, Depth_m, Distance, everything())
...
ggplot(interp_temp, aes(x=x, y=y))+
    ...
    geom_point(data = sensor_22may24, aes(x = Distance, y = 0.1, z = NULL), pch = 25, size = 3, fill = "black")+ #to mark cast sites 
