...
eems_hpb_cove <- eems_cove |> 
  ...
  filter(Date > ymd("2023-05-08"),
         Date < ymd("2023-08-08")) |> 
  select(1:6, HIX, BIX, DOC_mgL) |> 
  filter(Site_num %in% c(50,10,100,1000)) |> 
  pivot_longer(-c(1:6)) |> 
...
eems_hpb_ysi <- ysi |> 
  mutate(DateTime = mdy_hm(DateTime),
         DateTime = as.Date(DateTime)) |> 
    filter(Site_EEM %in% c(50,10,100,1000)) |> 
  select(-Notes) |> 
  pivot_longer(-c(1:5)) |> 
  filter(name %in% c("Temp_C", "DO_mgL", "SpCond_uScm")) |> 
...
rbind(eems_hpb_cove, eems_hpb_ysi) |> 
 mutate(Distance = ifelse(Site_num == 1000, "Stream", NA),
  ...
  filter(Depth_m %in% depths,
         name %in% c("BIX", "HIX", "DOC_mgL", "Temp_C")) |> 
ggplot(aes(x = factor(Distance, level = levels_distances), y = value, color = factor(Depth_m, levels = depths),
  ...
  geom_point(size = 2)+
  geom_line()+
  facet_grid(factor(name, levels = c("BIX", "HIX", "DOC_mgL", "Temp_C"))~Date, scales = "free_y")+
  labs(y = "value", x = "Site", color = "Depth (m)")+
  theme_bw()+ 
  theme(legend.position = "top", text = element_text(size = 16))+ guides(color = guide_legend(nrow = 1))+
  scale_color_manual(values = c("0.1" = "#000000", "0.5" = "red", "1" = "#E69F00", "6" = "#56B4E9", "9" = "#009E73", "20" = "#F0E442"))
...
ccr_met |> 
  ...
  ggplot(aes(x = DateTime, y = BP_Average_kPa))+
  geom_point()+
  #geom_line()+
  labs(x = "Date", y = "BP (kPa)")+
  theme_classic() + theme(text = element_text(size = 16))


roa_noaa <- read.csv("C:/Users/dwh18/Downloads/ROA_met_4apr24.csv") |> 
...
ccr_met_git |> 
  ...
  geom_point()+
  geom_point(data = roa_noaa, aes(x= DateTime, y = BP_inHg, color = "ROA met"))+
  #geom_line()+
