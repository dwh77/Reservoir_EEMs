...
eems_summary <- eems_plotting |> 
  ...
  rename(HIX = HIX.) |>  #### can just change the rename here for what value you want to look at
  select(Site, Depth_m, Date, Distance, Dry_start, Dry_end, HIX) |> 
...
levels_depthgroup <- c("0.1m", "0.2 - 1m",  "1 - 2m", "4 - 7m", "9m", " >= 10m")
...
eems_summary |> 
  mutate(depth_group = ifelse(Depth_m == 0.1, "0.1m", NA),
         ...
         depth_group = ifelse(Depth_m <= 7 & Depth_m >= 4, "4 - 7m", depth_group),
