...
data <- sensors |> 
  ...
  full_join(solubles_summary, by = c("Site", "Date", "Depth_m", "Site_Class", "Distance", "Dry_start", "Dry_end")) |>   left_join(CCR_usgs_data, by = "Date")

# data <- eems_summary

data2 <- plyr::rbind.fill(sensors, eems_summary, solubles_summary)
...
igc_wl <- CCR_usgs_data |> 
  ggplot(aes(x = Date, y = CCR_Dam_depth_m))+
  geom_point(size = 3)+
  ...
  labs(x= element_blank(), y = "Reservoir Water Level (m)")+
...
ggsave("./igc25_figures/DOC_HIX.png", igc_doc_hix, height = 8.5, width = 21, units = "in")


######### variables by date 
igc_dates <- data2 |> 
  filter(SpCond_uScm < 200) |> 
  select(Site, Date, Depth_m, Site_Class, Distance, Dry_start, Dry_end, Temp_C, SpCond_uScm, DOC_mean, HIX_mean) |> 
  pivot_longer(-c(1:7)) |> 
  ggplot(aes(x = Distance, y = value, fill = Depth_m,
             #shape = factor(Site_Class, levels = c("Stream", "Pool", "Cove", "Pelagic"))
             ))+
  ...
  facet_grid(factor(name, levels = c("Temp_C", "SpCond_uScm", "DOC_mean", "HIX_mean"))~Date, scales = "free_y")+
...
ggsave("./igc25_figures/acrossdates.png", igc_dates, height = 20, width = 21, units = "in")
