...
solubles_plotting <- left_join(ccr_solubles, distances, by = c("Site_code", "Date")) |> 
  ...
  rename(DOC = 10,
         Site = Site_number) |> 
  select(Reservoir, Site, Site_code, Site_Class, Depth_m, Date, Distance, Dry_start, Dry_end, DOC, NO3, NH4, PO4 )
...
solubles_summary |> 
  filter(!Site == 50) |> 
  ggplot(aes(x = Distance, y = DOC_mean, color = Depth_m, fill = Depth_m,
...
solubles_summary |> 
  filter(!Site == 50) |> 
  ggplot(aes(x = Distance, y = PO4_mean, color = Depth_m, fill = Depth_m,
             shape = factor(Site_Class, levels = c("Stream", "Pool", "Cove", "Pelagic"))))+
  geom_point() + 
...
solubles_summary |> 
  filter(!Site == 50) |> 
  ggplot(aes(x = Distance, y = NO3_mean, color = Depth_m, fill = Depth_m,
             shape = factor(Site_Class, levels = c("Stream", "Pool", "Cove", "Pelagic"))))+
  geom_point() + 
...
solubles_summary |> 
  filter(!Site == 50) |> 
  ggplot(aes(x = Distance, y = NH4_mean, color = Depth_m, fill = Depth_m,
             shape = factor(Site_Class, levels = c("Stream", "Pool", "Cove", "Pelagic"))))+
  geom_point() + 
