...
sensors |> 
  ...
  select(-DO_mgL) |> 
  pivot_longer(-c(1:9)) |> 
  ggplot(aes(x = Distance, y = value, color = Depth_m, fill = Depth_m,
...
##trying new color scale gray scale
sensors |> 
  filter(Site_code != "C1") |> 
  select(-DO_mgL) |> 
  pivot_longer(-c(1:9)) |> 
...
##trying new color scale red to blue; removed midpoint color and made outline (color) just black
sensors |> 
  filter(Site_code != "C1") |> 
  select(-DO_mgL) |> 
  pivot_longer(-c(1:9)) |> 
  ...
  # scale_color_gradient2(low = "darkred", mid = "moccasin", high ="darkblue", 
  #                      midpoint = 3,  guide = "colourbar")+
  # scale_fill_gradient2(low = "darkred", mid = "moccasin", high ="darkblue",  
  #                      midpoint = 3,  guide = "colourbar")


##below color gradient: https://stackoverflow.com/questions/17864233/customized-scale-color-gradient2-in-ggplot2
  # scale_color_gradient2(low = "#0000FF", mid = "#FFFFFF", high ="#FF0000",
  #                      midpoint = median(sensors$Depth_m),  guide = "colourbar")+
  # scale_fill_gradient2(low = "#0000FF", mid = "#FFFFFF", high ="#FF0000",
  #                      midpoint = median(sensors$Depth_m),  guide = "colourbar")

##DWH trial 
  # scale_color_gradient2(high = "red", mid = "green", low = "blue", breaks = c(0.1,10,20))+
  # scale_fill_gradient2(high = "red", mid = "green", low = "blue", breaks = c(0.1,10,20))
