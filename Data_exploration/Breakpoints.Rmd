...
plot_data <-  iso_plotting |> 
    filter(Depth_new %in% c("0.1")) |> 
  #filter(Site_code_new %in% c("S1", "B1", "C1", "P1")) |> 
...
# Scaling factor to match rain bars to plot height
scale_factor <- max_dexcess / max_rain

ggplot() +
  # Rain bars (scaled to match primary axis)
  geom_col(data = plot_data, aes(x = Date, y = Rain_mm * scale_factor),
           fill = "lightblue", width = 1) +

  # Isotope points and lines
  geom_point(data = plot_data, aes(x = Date, y = D_excess,
                                   color = factor(Site_code_new, levels = iso_levels_sites)),
             size = 3) +
  geom_line(data = plot_data, aes(x = Date, y = D_excess,
                                  color = factor(Site_code_new, levels = iso_levels_sites)),
            size = 1.3) +

  # Axis labels and annotation
  labs(x = "Date", y = "Deuterium excess", color = "Site", linetype = "Depth") +
  annotate("text", x = ymd("2024-06-01"), y = 18,
           label = "D excess = dD - 8*d18O", size = 5, color = "black", hjust = 0) +
  ...
  # Add secondary axis for rainfall
  scale_y_continuous(
    sec.axis = sec_axis(~  . / scale_factor, name = "Rainfall (mm)")
  )


ggsave("AGU25_figs/iso_SBCP_Rain.png", width = 9.5, height = 5.5, units = "in")
...
ggplot() +
  ...
  ylim(4, 18)+
  guides(color = guide_legend(nrow = 2, byrow = TRUE)) +
  scale_color_viridis_d(option = "H", drop = F) #drop makes so colors stay the same for plotting 


ggsave("AGU25_figs/iso_SBCP.png", width = 9, height = 5.5, units = "in")
...
```{r}
chem |> left_join(hydro_data, by= "Date") |> 
  ggplot(aes(x = Distance, y = BIX, fill = CCR_Dam_depth_m))+
    geom_point(size = 3, shape = 21)+ #need shape here if not doing site type
  theme_bw()+ theme(legend.position = "top", text = element_text(size = 18),
                    legend.text = element_text(size = 12),
                    panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
