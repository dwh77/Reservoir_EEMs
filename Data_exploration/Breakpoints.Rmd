...
#Faceting
chem |> 
  dplyr::select(-c(FI_mean, a350_mean)) |> 
  pivot_longer(-c(1:10)) |> 
...
#fit normal linear regression 
my.lm <- lm(BIX_mean ~ Distance, data = bix_connected)
summary(my.lm)
...
#fit normal linear regression 
my.lm <- lm(BIX_mean ~ Distance, data = bix_disconnected)
summary(my.lm)
...
#fit normal linear regression 
my.lm <- lm(BIX_mean ~ Distance, data = surface_df)
summary(my.lm)
...
#fit normal linear regression 
my.lm <- lm(BIX_mean ~ Distance, data = max_depth_df)
summary(my.lm)
...
levels <- c("DOC_mean", "BIX_mean", "HIX_mean", "NO3_mean", "D_excess")
...
levels <- c("DOC_mean", "BIX_mean", "HIX_mean", "d18O_VSMOW", "d2H_VSMOW", "D_excess")
