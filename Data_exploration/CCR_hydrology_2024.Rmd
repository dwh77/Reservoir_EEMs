...
add predicted hbp to plot 

```{r}
hpb_tinker_interp <- test |> select(Date, predicted_hpb_T) 

left_join(CCR_usgs_data, hpb_tinker_interp, by = "Date") |> 
  mutate(interp = ifelse(is.na(CCR_HPB_daily_waterlevel_cm), "Interp", "Observed")) |> 
  mutate(HPB = ifelse(is.na(CCR_HPB_daily_waterlevel_cm), predicted_hpb_T, CCR_HPB_daily_waterlevel_cm )) |> 
  filter(Date >= ymd("2024-05-01")) |> 
  filter(Date < ymd("2025-05-01")) |> 
  select(Date, interp, CCR_MetStation_daily_rain_mm, HPB, CCR_Dam_depth_m) |> 
  pivot_longer(-c(1:2)) |> 
  mutate(interp = ifelse(name != "HPB", "Observed", interp )) |> #fix interp column so just affects HPB where interp happened
  # mutate(value = ifelse(name == "HPB", log10(value), value)) |> 
  mutate(name = ifelse(name == "HPB", "HPB_stage_cm", name)) |> 
  ggplot(aes(x = Date, y = value, shape = interp))+
  geom_point()+
  ...
  geom_vline(xintercept = ymd("2024-10-28"))+  geom_vline(xintercept = ymd("2024-12-17"))+
  geom_vline(xintercept = ymd("2025-02-26"))+  geom_vline(xintercept = ymd("2025-04-16"))+
  facet_wrap(~factor(name, levels = c("CCR_MetStation_daily_rain_mm", "HPB_stage_cm", "CCR_Dam_depth_m")), ncol = 1, scales = "free_y")+
  scale_x_date(breaks = "1 months", date_labels = "%b")+
  labs(x= element_blank())+
  theme_bw()+ theme(text = element_text(size = 18),
                    panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
##export daily data frame for plotting w/ breakpoint script
hydro_data <- left_join(CCR_usgs_data, hpb_tinker_interp, by = "Date") |> 
  mutate(interp = ifelse(is.na(CCR_HPB_daily_waterlevel_cm), "Interp", "Observed")) |> 
  mutate(HPB = ifelse(is.na(CCR_HPB_daily_waterlevel_cm), predicted_hpb_T, CCR_HPB_daily_waterlevel_cm ))

#write.csv(hydro_data, "./Hydro_dailyData.csv", row.names = F)
